...
@interface BirthdayView ()<CAAnimationDelegate>
...
- (void)setLayerFrame {
    ...
    //盒子
    CGFloat bodyWith = self.birthdayBodyImage.size.width * self.uiScale;
    CGFloat bodyHeight = self.birthdayBodyImage.size.height * self.uiScale;
    self.birthdayBodyLayer.frame = CGRectMake(lidOriginX, CGRectGetMaxY(self.birthdayLidLayer.frame)-22, bodyWith, bodyHeight);
    ...
    CGFloat birthdayHearHeight = self.birthdayHeartImage.size.height * self.uiScale;
    CGFloat musicW = self.musicNoteImage.size.width * self.uiScale;
    CGFloat musicH = self.musicNoteImage.size.height * self.uiScale;
    CGRect musicOriginFrame = CGRectMake((self.width-musicW)/2, birthdayHearHeight, musicW, musicH);
    
    //音符起始位置
    self.musicNoteLeftLayer.frame     = musicOriginFrame;
    self.musicNoteRightOneLayer.frame = musicOriginFrame;
    self.musicNoteRightTwoLayer.frame = musicOriginFrame;
    [self initMusicNoteLeftLayerTransform3D];
...
//音符旋转
- (void)initMusicNoteLeftLayerTransform3D {
...
#pragma mark -- animationEvent 从小变大的动画
...
- (void)animationForBirthdayMusic {
    ...
    [left moveToPoint:originPoint];
    CGPoint leftControlPoint1 = CGPointMake(-10, self.height - 100);
    CGPoint leftControlPoint2 = CGPointMake(0, 0);
    [left addCurveToPoint:leftEndPoint controlPoint1:leftControlPoint1 controlPoint2:leftControlPoint2];
    [self animationForBirthdayMusicAnimation:left layer:self.musicNoteLeftLayer];
...
- (void)animationForBirthdayMusicAnimation:(UIBezierPath *)bezierPath layer:(CALayer *)layer {
    ...
    musicOpacityAnimation.fromValue = [NSNumber numberWithFloat:1.0f];
    musicOpacityAnimation.toValue   = [NSNumber numberWithFloat:0.0f];
...
-  (void)animationForBirthdayLid {
    ...
    //动画开始播放音乐
    [[AudioPlayerMgr sharedInstance] playMusic:@"birthday.mp3" isLoops:YES];
...
- (CAMediaTimingFunction *)animationTimingFunction {
    if (nil == _animationTimingFunction) {
        _animationTimingFunction = [CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseInEaseOut];//慢慢加速再慢慢减速
    }
...
- (void)removeAnimation:(CALayer *)subLayer {
    [subLayer removeAllAnimations];
    [subLayer removeFromSuperlayer];
    subLayer = nil;
}
